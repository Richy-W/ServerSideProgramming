...
switch ($action) {
    ...
    case 'process_data':
        ...
        if(empty($invoice_date_s) || empty($due_date_s)) {
            $message = "Please enter both dates and try again.";
            break;
        }
        ...
        try {
            ...
            $message ="Please enter the dates in a valid formant mm/dd/yyyy.";
            break;
        }

        // make sure the due date is after the invoice date
        if ($due_date_o < $invoice_date_o) {
            $message = "Due date must come after the initial invoice date.";
            break;
        }
        ...
        $timeDiff = $current_date_o->diff($due_date_o);
        if ($due_date_o > $current_date_o) {
            $due_date_message = $timeDiff->format('This Invoice is due in %y years, %m months, and %d days.');
        }
        else {
            $due_date_message = $timeDiff->format('This Invoice is %y years, %m months, and %d days overdue.');
        }
